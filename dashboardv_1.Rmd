...
selectInput('ctryInput', 'Country', choices = gather_table$Country, selected = "US")
...
```{r}
dataset <- reactive({
   gather_table %>% filter(Country == input$ctryInput) %>%group_by(Date,Status) %>% summarise(Affected = sum(Count))
})



  renderHighchart({
    h1<- hchart(dataset(), "line", hcaes(x = Date, y = Affected, group = Status)) %>% hc_title(text = "Timeline",
           margin = 20, align = "center",
           style = list( useHTML = TRUE))
