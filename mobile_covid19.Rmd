...
latest_ctryConfirmed <- reactive({
  ...
  arrange(desc(Date)) %>% 
  head(1) %>% ungroup() %>%
  select(confirmed) %>% pull()
})

latest_ctrydeath <- reactive({
  ctry_spread %>%
  spread(key = Status, value = Affected) %>%
  filter(Country == input$ctryInput) %>% 
  arrange(desc(Date)) %>% 
...
### Confirmed Cases {.value-box} {.mobile}
...
### Dead {.value-box} {.mobile}

```{r}
renderValueBox({
  
  ctrydead <- latest_ctrydeath()
  valueBox(
    value = ctrydead,
    icon = "fas fa-exclamation-triangle",
    color = "#EC7063"
...
valueBox(value = today_change_ctry,
...
### Fatality Rate {.value-box} {.mobile}
